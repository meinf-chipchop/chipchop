  ...
  const handleAddItem = (dish: Dish) => {
    ...
    if (added)
      handleToast(
        ...
        "Dish " + dish.name + " is ready for checkout.",
        "success"
      );
    else
      handleToast("Something went wrong!", "Please check the cart.", "warning");
  };
      ...
      <View className="flex flex-row items-center gap-x-2 w-full mb-2">
        ...
        <View className="flex-1 mx-2">
          <View className="flex flex-row gap-x-2">
            <Text className="font-bold text-md text-gray-800 align-baseline">
              {dish.name}
            </Text>
            {!isExpanded && dish.rating_average && (
              <Text className="text-sm text-gray-500">
                {dish.rating_average}/5 ({dish.rating_count})
              </Text>
            )}
          </View>
          <View className="flex flex-row gap-x-2">
      ...
      {isExpanded && (
        <View className="flex flex-col gap-2 mb-2">
          ...
          <View className="w-auto align-center items-center bg-gradient-to-br from-primary-500 to-primary-700 rounded-md py-2">
            {dish.rating_average !== undefined &&
            Number(dish.rating_average) != 0 ? (
              <RatingCard
          ...
          <View className="flex flex-row gap-x-2 m-auto">
            ...
            {dish.estimated_time && (
              <Text className="text-xs italic">
                {t("dish.estimated_time")}:{" "}
                <b className="text-md">{dish.estimated_time}</b>
              </Text>
            )}
          </View>
        </View>
