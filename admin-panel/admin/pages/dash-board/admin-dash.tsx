...
const MenuItem: React.FC<MenuItemProps> = ({ icon: Icon, text, isActive, onClick, subItems, isExpanded }) => (
  <li>
    <div
    ...
    >
      ...
      {subItems && (isExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />)}
    </div>
    {subItems && isExpanded && (
...
export default function AdminDashboard() {
  ...
  const handlePermission = (type: 'cooks' | 'deliverers', id: number, status: 'approved' | 'denied') => {
    setPermissions((prev) => {
    ...
    if (activeItem === 'Cooks' || activeItem === 'Deliverers') {
      ...
      return (
        <div className="bg-gray-800 rounded-lg p-6">
          <h3 className="text-xl font-semibold mb-4 text-white">{activeItem} List</h3>
              ...
              {visibleUsers.map((user) => (
                <tr key={user.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                    <span className={`px-2 py-1 rounded-full text-xs ${user.status === 'approved' ? 'bg-green-200 text-green-900' :
                        user.status === 'denied' ? 'bg-red-200 text-red-900' :
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button
                      ...
                      onClick={() => handlePermission(type, user.id, 'approved')}
                    >
                      <Check size={16} />
                    </button>
                    <button
    ...
    return (
      <div className="bg-gray-800 rounded-lg p-6">
        <p className="text-gray-300">
          ...
          Here you can manage and view important information related to {activeItem.toLowerCase()}.
        </p>
      </div>
    );
  };
          ...
          <ul className="space-y-2 py-4">
            {menuItems.map((item) => (
              <MenuItem
                ...
                icon={item.icon}
                text={item.text}
                isActive={!!(activeItem === item.text || (item.subItems && item.subItems.some(subItem => subItem.text === activeItem)))}
